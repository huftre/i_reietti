(() => {
  ...
  async function searchPlayers(query, force = false, extended = false) {
    ...
    try {
      ...
      results.innerHTML = `
        <div class="sos-search-error">
          <strong>Ricerca non disponibile.</strong>
          <span>${escapeHtml(friendlyError(error))}</span>
        </div>
      `;
  ...
  function renderSearchResults(
    ...
    */
    ...
    const rowsHtml = visible.map((item, index) => {
      ...
      return `
        <button
          class="sos-search-result${verified ? ' verified' : ''}"
          type="button"
          data-player-index="${index}"
        >
          <img
            src="${escapeAttr(player.photo || '')}"
            alt=""
            loading="lazy"
          >
          ...
          <span class="sos-result-copy">
            ...
            <small>
              ${escapeHtml(meta || 'Dati disponibili nella scheda')}
            </small>

            ${
              tags
                ? `<span class="sos-result-tags">${tags}</span>`
                : ''
            }
          </span>

          <span
            class="sos-result-arrow"
            aria-hidden="true"
          >
            →
          </span>
    ...
    const footerHtml = hiddenCount
      ...
      : (
          extended
            ? `
              <div class="sos-search-mode-note">
                Risultati dell’archivio esteso, ordinati per pertinenza.
              </div>
            `
            : ''
        );
    ...
    container
      ...
      .forEach(button => {
        button.addEventListener('click', () => {
          const item =
            visible[Number(button.dataset.playerIndex)];
    ...
    container
      .querySelector('[data-sos-show-more]')
  ...
  async function openPlayer(searchItem) {
    const modal =
      document.querySelector('#sos-player-modal');

    const content =
      document.querySelector('#sos-player-modal-content');
    ...
    content.innerHTML = `
      <div class="sos-modal-loading">
        ...
        <small>
          Analizzo rendimento, titolarità, ruolo e contesto attuale.
        </small>
      ...
      */
      ...
      content.innerHTML = `
        <div class="sos-detail-error">
          ...
          <h2>
            Non riesco a completare la scheda
          </h2>
  ...
  function renderPlayerDetail(
    ...
    /*
       Il Worker mette in "current"
       la migliore base statistica disponibile.
    */
    const primaryAgg =
      aggregateStatistics(
    ...
    const secondaryAgg =
      aggregateStatistics(
        secondaryItem?.statistics || []
      );

    const rawRole =
      primaryAgg.position ||
      pickCurrentStat(
        searchItem.statistics || []
      )?.games?.position ||
    ...
    const sourceLabel =
      detail.sources?.statistics ||
      detail.historical?.source ||
      'dati disponibili';

    const seasonLabel =
      dataSeason
        ? seasonText(dataSeason)
        : 'stagione disponibile';
    ...
    content.innerHTML = `
      ...
      <div class="sos-player-head">
        ...
        <div class="sos-player-photo-wrap">
          ...
          <img
            class="sos-player-photo"
            src="${escapeAttr(player.photo || '')}"
            alt="${escapeAttr(player.name || 'Giocatore')}"
          >

          <span class="sos-role-pill">
        ...
        <div class="sos-player-identity">
          ...
          <span class="eyebrow">
            SOS Reietti · Informazioni per asta
          </span>

          <h2 id="sos-player-name">
            ${escapeHtml(
              player.name ||
              `${player.firstname || ''} ${player.lastname || ''}`.trim()
            )}
          ...
          ${
            player.injured
              ? `
                <div class="sos-injury-warning">
                  ⚠️ Segnalato come indisponibile/infortunato nei dati disponibili.
                </div>
              `
              : ''
          }

          <div class="sos-profile-tags">
            ${
              evaluation.tags
                .map(
                  tag =>
      ...
      <div class="sos-price-panel">
        ...
        <div class="sos-price-block">
          ...
          <!--
            MODIFICA QUI:
            titolo della cifra da cui partire in asta.
          -->
          <span>
            BASE D'ASTA CONSIGLIATA
          </span>
      ...
      <div class="sos-score-grid">
        ...
        ${scoreCard(
          'Qualità',
          evaluation.components.quality
        )}

        ${scoreCard(
          'Affidabilità',
          evaluation.components.reliability
        )}

        ${scoreCard(
          evaluation.componentsLabel,
          evaluation.components.bonus
        )}

        ${scoreCard(
          'Disciplina',
          evaluation.components.discipline
        )}
      ...
      <section class="sos-detail-section">
        ...
        <div class="sos-detail-title">
          ...
          <span
            class="sos-confidence ${evaluation.confidenceClass}"
          >
            ${escapeHtml(evaluation.confidence)}
          </span>
        ...
        <div class="sos-stat-grid">
          ...
          ${statItem(
            'Rating',
            formatRating(primaryAgg.rating)
          )}
      ...
      <section class="sos-detail-section">
        ...
        <p class="sos-explanation sos-profile-summary">
      ...
      <section class="sos-detail-section">
        ...
        <div class="sos-detail-title">
          ...
          <div>
            ...
            <h3>
              Serie A ${escapeHtml(seasonText(live))}
            </h3>
      ...
      <!--
        ...
        Budget della Lega / Come nasce il prezzo
      -->


      <div class="sos-model-note">

        Il prezzo SOS è un riferimento per l'asta iniziale:
        non è una quotazione ufficiale e non garantisce rendimento futuro.
  ...
  function renderCurrentContext(
    ...
    const dateText =
      next?.date
        ? formatItalianDate(next.date)
        : '—';

    return `
      <div class="sos-current-grid">

        <div class="sos-current-card">
        ...
        <div class="sos-current-card wide">
  ...
  /* =========================================================
     ...
     - estimateAuctionPrice()
       trasformazione indice -> crediti.

     - contextAdjustment()
       piccolo correttivo sul contesto attuale.
  ...
  function evaluatePlayer({
    ...
    if (primaryAgg.minutes > 0) {
      ...
      if (
        secondaryAgg.minutes > 500
      ) {
        index =
          primary.index * 0.78 +
          secondary.index * 0.22;
      }
    } else {
      index = 50;
    }
  ...
  function estimateAuctionPrice(
    ...
    /*
       ...
       Sono quote del budget totale
       prima del correttivo scarsità.
    */
    const ceilingShare = {
      P: 0.075,
      D: 0.150,
    ...
    */
    ...
    /*
       Questi valori restano interni
       all'algoritmo.

       La "fascia prudente"
       NON viene più mostrata nel popup.
    */
    const spread =
  ...
  function freshnessAdjustment(
    ...
    if (
      !Number.isFinite(dataSeason) ||
      !Number.isFinite(liveSeason)
    ) {
      return 0;
    }
  ...
  function freshnessNote(
    ...
    return `
      La base individuale è storica
      (${gap} stagioni di distanza):
      il modello applica un piccolo correttivo prudenziale
      e usa squadra/calendario attuali per il contesto.
    `.replace(/\s+/g, ' ').trim();
  }
  ...
  function buildSummary(
    ...
    if (role === 'A') {
      profile =
        `${stats.goals} gol e ${stats.assists} assist nella base statistica, ` +
        `${per90(stats.goals).toFixed(2)} gol ogni 90'.`;

    } else if (
      role === 'C'
    ) {
      profile =
        `${stats.goals} gol e ${stats.assists} assist, con ` +
        `${per90(stats.keyPasses).toFixed(2)} passaggi chiave ogni 90'.`;

    } else if (
      role === 'D'
    ) {
      profile =
        `${stats.goals} gol e ${stats.assists} assist, più ` +
        `${per90(
          stats.tackles +
          stats.interceptions +
          stats.blocks
        ).toFixed(2)} interventi difensivi ogni 90'.`;

    } else {
      profile =
        `${stats.saves} parate e ${stats.conceded} gol subiti nella base statistica.`;
    }
    ...
    const contextText =
      next
        ? ` Prossimo impegno: ${next.opponent || 'avversario da definire'} ${
            next.venue === 'casa'
              ? 'in casa'
              : 'in trasferta'
          }.`
  ...
  function priceExplanation(
    ...
    if (
      ...
      return `
        Profilo di fascia alta:
        in una lega a 14 la scarsità dei ${roleName} forti
        fa salire rapidamente il prezzo.
        Il modello conserva comunque una riserva
        per completare tutti i ${info.roleSlots} posti del reparto.
      `.replace(/\s+/g, ' ').trim();
    }
    ...
    if (
      ...
      return `
        Investimento importante ma non da all-in.
        Il prezzo tiene conto della concorrenza di 14 squadre
        e di un budget indicativo di circa
        ${info.roleBudget} crediti per il reparto.
      `.replace(/\s+/g, ' ').trim();
    }
    ...
    if (
      ...
      return `
        Prezzo da buon titolare o rotazione:
        il modello protegge il budget per gli slot più costosi
        del reparto e limita le rincorse d'asta.
      `.replace(/\s+/g, ' ').trim();
    }

    return `
      Profilo da scommessa:
      con 25 giocatori da acquistare conviene tenere il costo contenuto
      e conservare crediti per gli indici più alti.
    `.replace(/\s+/g, ' ').trim();
  }
