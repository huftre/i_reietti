(() => {
  ...
  async function searchPlayers(query) {
    ...
    try {
      ...
      setHelp(
        players.length
          ? `${players.length} risultato${players.length === 1 ? '' : 'i'} trovato${players.length === 1 ? '' : 'i'}${suffix}.`
          : (
              data.fallbackError
  ...
  function renderSearchResults(players) {
    ...
    container.innerHTML = players.map((item, index) => {
      ...
      const team = String(stat?.team?.name || '').trim();

      /*
         MODIFICA v4:
         i risultati della ricerca estesa API-Football possono non
         avere ancora la squadra dentro l'autocomplete. La squadra
         corrente viene risolta automaticamente quando apri la scheda.
      */
      const meta = [
        team && team !== '—'
          ? team
