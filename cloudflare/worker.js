...
function candidateScore(item, player) {
  ...
  if (a === b) score += 8;
  else if (a.includes(b) || b.includes(a)) score += 6;
  else score += tokenSimilarity(a, b) * 5;
...
function mergeIdentity(apiItem, currentIdentity, tsdPlayer) {
  const stats = Array.isArray(apiItem?.statistics)
    ? apiItem.statistics.map(stat => ({
        ...
        games: {
          ...(stat?.games || {}),
          // Manteniamo il ruolo attuale del giocatore.
...
async function serieAContext(playerTeam, env, origin, ctx) {
  ...
  if (!common) {
    try {
      const competition = encodeURIComponent(competitionCode(env));

      const [standingsData, matchesData] = await Promise.all([
        fdGet(`/competitions/${competition}/standings`, env),
        fdGet(`/competitions/${competition}/matches?status=SCHEDULED`, env)
      ]);

      const block =
        standingsData?.standings?.find(s => String(s?.type).toUpperCase() === 'TOTAL') ||
        standingsData?.standings?.[0];

      common = {
        standings: Array.isArray(block?.table) ? block.table : [],
        matches: Array.isArray(matchesData?.matches) ? matchesData.matches : []
      };

      const commonResponse = new Response(JSON.stringify(common), {
        headers: {
      ...
      cachePut(cacheKey, commonResponse, ctx);

    } catch (error) {
      return {
        competition: competitionCode(env),
        configured: true,
        error: error?.message || 'football-data non disponibile.',
        team: null,
        standing: null,
        nextMatch: null
      };
  ...
  const standing = bestStanding(playerTeam, common.standings);
  ...
  const next = common.matches
    ...
    .sort((a, b) => new Date(a.utcDate || 0) - new Date(b.utcDate || 0))[0] || null;

  const home = next?.homeTeam?.id === team.id;
...
async function fdGet(path, env) {
  ...
  if (!response.ok) {
    throw new Error(
      `football-data.org: ${data?.message || `HTTP ${response.status}`}`
    );
  }
...
async function tsdGet(path, env) {
  const key = String(env.THESPORTSDB_KEY || '123').trim();
  const url = `${TSD_BASE}/${encodeURIComponent(key)}${path}`;
...
function identity(p) {
  const name = p?.strPlayer || 'Giocatore';
  const words = String(name).trim().split(/\s+/);

  return {
    id: numberOrNull(p?.idPlayer),
    name,
    firstname: words[0] || '',
    lastname: words.slice(1).join(' '),
    age: ageFromDate(p?.dateBorn),
    birth: {
      date: p?.dateBorn || null,
      place: p?.strBirthLocation || null,
      country: p?.strNationality || null
    },
    nationality: p?.strNationality || null,
    height: p?.strHeight || null,
    weight: p?.strWeight || null,
    injured: false,
    photo: p?.strCutout || p?.strThumb || p?.strRender || ''
  };
}

function emptyCurrent(player, currentIdentity, env) {
  return {
    player: currentIdentity,
    statistics: [{
      team: {
        id: numberOrNull(player?.idTeam),
        name: player?.strTeam || '—',
        logo: null
      },
      league: {
      ...
      games: {
        ...
        position: position(player?.strPosition),
        rating: null,
        captain: false
      },
...
function bestStanding(playerTeam, rows = []) {
  ...
  for (const row of rows) {
    ...
    for (const candidate of [team.name, team.shortName, team.tla].filter(Boolean)) {
      const score = teamSimilarity(playerTeam, candidate);
...
function teamSimilarity(a, b) {
  ...
  const A = new Set(teamTokens(x));
  const B = new Set(teamTokens(y));
...
function teamName(value) {
  ...
  const aliases = [
    [/^fc internazionale milano$|^internazionale$|^inter milan$|^inter$/, 'inter'],
    [/^ac milan$|^milan$/, 'milan'],
    [/^ssc napoli$|^napoli$/, 'napoli'],
    [/^juventus fc$|^juventus$/, 'juventus'],
    [/^as roma$|^roma$/, 'roma'],
    [/^ss lazio$|^lazio$/, 'lazio'],
    [/^acf fiorentina$|^fiorentina$/, 'fiorentina'],
...
function teamTokens(value) {
  const ignored = new Set([
    'fc', 'ac', 'as', 'ss', 'ssc', 'calcio', 'football', 'club'
  ]);
...
function position(value) {
  ...
  if (p.includes('goalkeeper') || p.includes('keeper') || p === 'gk') {
    return 'Goalkeeper';
  }
...
function liveSeason(env) {
  ...
  return Number(env.CURRENT_SEASON || 2026);
}

function historicalSeason(env) {
  // MODIFICA QUI solo quando API-Football Free abilita una stagione più recente.
  return Number(env.API_FOOTBALL_PRIMARY_SEASON || 2024);
}

function serieAId(env) {
  // Serie A su API-Football.
  return Number(env.API_FOOTBALL_LEAGUE_ID || 135);
}

function competitionCode(env) {
  // Serie A su football-data.org.
  return String(env.FOOTBALL_DATA_COMPETITION || 'SA').trim();
}

function allowedOrigin(env) {
  // Per https://huftre.github.io/i_reietti/ lascia questo valore.
  return String(env.ALLOWED_ORIGIN || 'https://huftre.github.io')
    .replace(/\/$/, '');
}
...
function corsHeaders(request, env) {
  ...
  return {
    'Access-Control-Allow-Origin':
      origin && originAllowed(request, env)
        ? origin
        : allowedOrigin(env),
...
function cachePut(key, response, ctx) {
  try {
    const promise = caches.default.put(key, response).catch(() => {});
